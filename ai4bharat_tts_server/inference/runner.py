...
class ParlerTTSModelRunner:
    ...
    def prefill(self, request):
        ...
        # Register only after prefill fully succeeds, so every pid in
    ...
    def check_stopping_criteria(self):
        ...
        # Single GPU->CPU sync for the whole batch (avoid per-request .item()).
        stacked = torch.stack(
            [self.running_requests[pid].decoder_input_ids[-1] for pid in ordered_pids],
            dim=0,
        )  # (batch, num_codebooks, 1)
        eos_flags = (
            (stacked == self.eos_token_id)
            .reshape(len(ordered_pids), -1)
            .all(dim=1)
            .tolist()
        )
        to_evict = []
        for pid, is_eos in zip(ordered_pids, eos_flags):
            req = self.running_requests[pid]
            if is_eos:
                to_evict.append(req)
                continue
