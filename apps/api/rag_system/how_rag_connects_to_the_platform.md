...
- **Compose:** `../../../legacy/docker-compose.yml` mounts `./voicera_backend:/app` for dev so code edits apply; Chroma data usually ends up under `voicera_backend/rag_system/chroma_data/...` on the **host** unless you set **`CHROMA_BASE_DIR`** elsewhere.
