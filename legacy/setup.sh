...
if [ "$ENABLE_STT" = "yes" ]; then
  STT_DIR="$REPO_DIR/model-server/STT/indic-conformer"
...
if [ "$ENABLE_TTS" = "yes" ]; then
  TTS_DIR="$REPO_DIR/model-server/TTS/ai4bharat"
...
# V2V
V2V_DIR="$REPO_DIR/apps/voice-runtime"
...
# Frontend
if [ ! -d "$REPO_DIR/apps/dashboard/node_modules/.bin/next" ] && [ ! -f "$REPO_DIR/apps/dashboard/node_modules/.bin/next" ]; then
  cd "$REPO_DIR/apps/dashboard" && npm install --silent 2>&1 | tail -2
else
...
tmux send-keys -t voicera:vllm 'source $REPO_DIR/model-server/LLM/venv/bin/activate && vllm serve Qwen/Qwen3-8B --port 8003 --host 0.0.0.0 --enable-auto-tool-choice --tool-call-parser hermes' Enter
STARTEOF
...
tmux send-keys -t voicera:frontend "cd \$REPO_DIR/apps/dashboard && npx next dev --port 3000" Enter
...
echo 'NEXT_PUBLIC_JOHNAIC_SERVER_URL="https://PENDING"' > "$REPO_DIR/apps/dashboard/.env.local"
...
for i in $(seq 1 12); do
  ...
  [ "$i" = "1" ] && tmux send-keys -t voicera:frontend "cd $REPO_DIR/apps/dashboard && node_modules/.bin/next dev --port 3000" Enter
  sleep 5
done
