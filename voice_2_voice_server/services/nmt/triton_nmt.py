...
class NmtClient:
        ...
        while True:
            ...
            while len(batch) < self._max_batch:
                ...
                try:
                    ...
                    break
                batch.append(row)
            # Dispatch without awaiting so the next window opens immediately; the
            # semaphore inside _dispatch bounds real concurrency.
    ...
    async def _handle_batch_failure(self, batch: List[_Row], error: str) -> None:
        ...
        rooms' languages are unaffected. A batch of one has nothing to isolate.
        """
        if len(batch) <= 1:
            for r in batch:
                if not r.future.done():
                    r.future.set_exception(NmtError(error))
            return
